...
\begin{document}
  ...
  \section{Introduction}
    ...
    \begin{align}
        \control(T) = p \error(T) + d \dot{\error}(T) + i \int_0^T \error(t) \dt. \label{eq:control_equation}
    \end{align}
  ...
  \section{PIDNN control examples}
    ...
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=1.0\textwidth]{code/demo1.pdf}
        \subcaption{Constant Setpoint}
        \label{fig:demo:converging}
    \end{subfigure}
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=1.0\textwidth]{code/demo2.pdf}
  ...
  \subsection{Global Loss Minimization}
    ...
    Let now $\omega=\sqrt{\beta-\gamma^2}$ and  $\kappa_{1,2} = \gamma \pm i \omega$, then the solution for 
    $\state(0) = 0$ and $\dot{\state}(0) = 0$ becomes
    \begin{align}
        \state(t) = \frac{c p \setpoint}{\beta} \left( e^{\gamma} \cos(\omega t) - 1 \right).
    \end{align}

    With this, we can derive the loss as 
    \begin{align}
        L &= \int_0^{\infty} \frac{1}{2}\left( \frac{c p \setpoint}{b - cp} \left(e^\gamma \cos ( \omega t ) - \frac{b}{c p}\right) \right)^2 \dt.
    \end{align}
   ...
   As $\gamma$ is independent of $p$
    \begin{align}
        \pdiv{L}{p} = \pdiv{L}{\omega} \cdot \pdiv{\omega}{p},
    \end{align}
    where 
    \begin{align}
         \pdiv{L}{\omega} &= \frac{\gamma \omega}{2 (\gamma^2 + \omega^2)^2} \cdot \frac{\setpoint^2}{2} < 0\\
         \pdiv{\omega}{p} &= \frac{2 c}{\omega} > 0.
    \end{align}
  ...
  \subsection{Difference Quotient}
    ...
    \begin{figure}[tb]
        \centering
        \includegraphics[width=0.75\textwidth]{code/wcc.pdf}
  ...
  \subsection{Discussion}
    ...
    $\error^\prime(t) = \lambda \error(t/\lambda)$ givens $\loss[\error] = \loss[\error^\prime]$, but from an 
    engineering point of view the system behaviour corresponding to $\error^\prime$ can be arbitrarily bad: The error
    decay is sped up by a factor of lambda, but the overshoot is increased by the same factor. 
  ...
  \section{Simulation Results}
    ...
    \begin{figure}[htb]
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=1.0\textwidth]{code/compare_rect.pdf}
        \subcaption{Control with PID and both variations of PIDNN.}
        \label{subfig:sim:control}
    \end{subfigure}
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=1.0\textwidth]{code/compare_weights.pdf}
        \subcaption{Development of Weights. Dashed: Taylor, Solid: Resilient}
        \label{subfig:sim:weights}
    \end{subfigure}
    \caption{Simulation of PIDNN control of the system described by eq. \ref{eq:demo_system}.}
    \label{fig:weights}
    \end{figure}
    ...
    \begin{figure}[tb]
        \centering
        \includegraphics[width=1.0\textwidth]{code/quiver.pdf}
        ...
        controller, where dark/blue indicates low error. Data points which do not have an arrow indicate that the 
        weight changes become too big, or even diverged.}
        \label{fig:quiver}
    \end{figure}
